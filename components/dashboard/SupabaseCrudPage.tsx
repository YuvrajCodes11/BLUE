...
export function SupabaseCrudPage({ moduleKey }: { moduleKey: keyof typeof configs }) {
  ...
  return (
    <section>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-sm uppercase tracking-[0.24em] text-cyan-500">Supabase module</p><h1 className="mt-2 text-3xl font-bold text-[var(--text)] md:text-5xl">{config.title}</h1><p className="mt-3 max-w-3xl text-[var(--muted-text)]">{config.description}</p></div>{canCreateRecords && <button onClick={() => { setEditing(null); form.reset(); }} className="ocean-action rounded-xl px-4 py-2 font-bold text-slate-950">New record</button>}</div>
