  ...
  return (
    <div className="mt-8 grid gap-6 xl:grid-cols-[1fr_0.45fr]">
      <section className="rounded-2xl border border-white/10 bg-slate-950/45 p-5">
        ...
        <div className="mt-5 overflow-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.2em] text-slate-400"><tr><th className="p-3">Name</th><th className="p-3">Role</th><th className="p-3">County</th><th className="p-3">Status</th><th className="p-3">Actions</th></tr></thead><tbody>{profiles.map((profile)=><tr key={profile.id} className="border-t border-white/10"><td className="p-3 text-white">{profile.full_name}</td><td className="p-3"><select value={profile.role} onChange={(event)=>void updateProfile(profile.id,{ role: event.target.value as DbRole })} className="rounded-lg border border-white/10 bg-[#07111f] px-2 py-1 text-white">{roles.map((role)=><option key={role}>{role}</option>)}</select></td><td className="p-3 text-slate-300">{profile.county ?? "-"}</td><td className="p-3 text-slate-300">{profile.is_active === false ? "Inactive" : "Active"}</td><td className="p-3"><button onClick={()=>void updateProfile(profile.id,{ is_active: profile.is_active === false })} className="text-cyan-200">{profile.is_active === false ? "Activate" : "Deactivate"}</button></td></tr>)}</tbody></table></div>
      </section>
      <section className="rounded-2xl border border-white/10 bg-slate-950/45 p-5">
        <h2 className="text-xl font-bold">Audit logs</h2>
        <div className="mt-5 grid gap-3">{logs.length ? logs.map((log)=><div key={log.id} className="rounded-xl border border-white/10 bg-white/[0.04] p-3"><p className="text-sm text-white">{log.action}</p><p className="text-xs text-slate-400">{log.entity_type} · {log.entity_id ?? "-"}</p></div>) : <EmptyState title="No audit logs" body="Admin changes will be recorded here." />}</div>
      </section>
    </div>
  );
}
