...
export function SetPasswordCard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-6 py-24 text-white">
      <form onSubmit={submit} className="mx-auto max-w-md rounded-2xl border border-white/10 bg-white/[0.06] p-8 shadow-2xl shadow-cyan-950/30">
        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-cyan-300">BLUE Access</p>
        ...
        <div className="mt-6 grid gap-4">
          <input
            ...
            className="rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-white outline-none focus:border-cyan-300"
          />
          <input
            type="password"
        ...
        {error && <div className="mt-4 rounded-xl border border-rose-300/30 bg-rose-300/10 p-3 text-sm text-rose-100">{error}</div>}
      </form>
    </div>
  );
}
